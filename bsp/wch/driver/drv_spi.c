...
static struct mr_spi_bus spi_bus_device[MR_ARRAY_SIZE(ch32_spi_bus_data)];
...
static void ch32_spi_cs_write(mr_spi_bus_t spi_bus, mr_pos_t cs_number, mr_uint8_t value)
{
    GPIO_WriteBit(PIN_STPORT(cs_number), PIN_STPIN(cs_number), value);
}

static mr_uint8_t ch32_spi_cs_read(mr_spi_bus_t spi_bus, mr_pos_t cs_number)
{
    return GPIO_ReadOutputDataBit(PIN_STPORT(cs_number), PIN_STPIN(cs_number));
}
...
mr_err_t drv_spi_bus_init(void)
{
    ...
    mr_size_t count = MR_ARRAY_SIZE(spi_bus_device);
