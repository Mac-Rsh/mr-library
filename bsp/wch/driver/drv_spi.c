...
static mr_uint16_t ch32_spi_baud_rate_prescaler(mr_uint32_t pclk_freq, mr_uint32_t baud_rate)
{
	...
	if (psc >= 256) return SPI_BaudRatePrescaler_256;
	else if (psc >= 128) return SPI_BaudRatePrescaler_128;
	else if (psc >= 64) return SPI_BaudRatePrescaler_64;
	else if (psc >= 32) return SPI_BaudRatePrescaler_32;
	else if (psc >= 16) return SPI_BaudRatePrescaler_16;
	else if (psc >= 8) return SPI_BaudRatePrescaler_8;
	else if (psc >= 4) return SPI_BaudRatePrescaler_4;
	else if (psc >= 2) return SPI_BaudRatePrescaler_2;
...
static mr_err_t ch32_spi_configure(mr_spi_bus_t spi_bus, struct mr_spi_config *config)
{
	...
	switch (config->mode)
	{
		case MR_SPI_MODE_0: SPI_InitStructure.SPI_CPOL = SPI_CPOL_Low;
			SPI_InitStructure.SPI_CPHA = SPI_CPHA_1Edge;
			break;
		case MR_SPI_MODE_1: SPI_InitStructure.SPI_CPOL = SPI_CPOL_Low;
			SPI_InitStructure.SPI_CPHA = SPI_CPHA_2Edge;
			break;
		case MR_SPI_MODE_2: SPI_InitStructure.SPI_CPOL = SPI_CPOL_High;
			SPI_InitStructure.SPI_CPHA = SPI_CPHA_1Edge;
			break;
		case MR_SPI_MODE_3: SPI_InitStructure.SPI_CPOL = SPI_CPOL_High;
			SPI_InitStructure.SPI_CPHA = SPI_CPHA_2Edge;
			break;
		default: SPI_InitStructure.SPI_CPOL = SPI_CPOL_Low;
			SPI_InitStructure.SPI_CPHA = SPI_CPHA_1Edge;
			break;
	}
	...
	{
		case MR_SPI_BIT_ORDER_LSB: SPI_InitStructure.SPI_FirstBit = SPI_FirstBit_LSB;
			break;
		case MR_SPI_BIT_ORDER_MSB: SPI_InitStructure.SPI_FirstBit = SPI_FirstBit_MSB;
			break;
		default: SPI_InitStructure.SPI_FirstBit = SPI_FirstBit_MSB;
			break;
	}
