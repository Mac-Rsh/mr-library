...
static ssize_t _serial_read_poll(struct mr_serial *serial, uint8_t *buf, size_t count)
{
    struct mr_driver *driver = _MR_DEVICE_DRIVER_GET((struct mr_device *)serial);
...
static ssize_t _serial_read_fifo(struct mr_serial *serial, uint8_t *buf, size_t count)
{
    /* Read data from FIFO */
    return (ssize_t)mr_fifo_read(&serial->rfifo, buf, count);
}

static ssize_t _serial_write_poll(struct mr_serial *serial, const uint8_t *buf, size_t count)
{
    struct mr_driver *driver = _MR_DEVICE_DRIVER_GET((struct mr_device *)serial);
    struct mr_serial_driver_ops *ops = _MR_DRIVER_OPS_GET(driver);
    ssize_t wcount;
...
static ssize_t _serial_write_fifo(struct mr_serial *serial, const uint8_t *buf, size_t count)
{
    struct mr_driver *driver = _MR_DEVICE_DRIVER_GET((struct mr_device *)serial);
    struct mr_serial_driver_ops *ops = _MR_DRIVER_OPS_GET(driver);
    ...
    if (ops->send_dma != NULL)
    {
        if (MR_BIT_IS_SET(serial->state, _STATE_SEND_DMA) == false)
        {
            ...
            size_t wdma_count = mr_fifo_peek(&serial->wfifo, &serial->wdma, sizeof(serial->wdma));
...
static int _serial_fifo_allocate(struct mr_fifo *fifo, size_t *size)
{
...
static int serial_close(struct mr_device *device)
{
    ...
    if (MR_BIT_IS_SET(serial->state, _STATE_RECEIVE_DMA) == true)
    {
        ...
        MR_BIT_CLR(serial->state,
                   _STATE_RECEIVE_DMA | _STATE_RECEIVE_DMA_TOP | _STATE_RECEIVE_DMA_BOT);
    }
#endif /* MR_USE_SERIAL_DMA */
...
static ssize_t serial_read(struct mr_device *device, int pos, void *buf, size_t count)
{
    struct mr_serial *serial = (struct mr_serial *)device;
...
static ssize_t serial_write(struct mr_device *device, int pos, const void *buf, size_t count)
{
    struct mr_serial *serial = (struct mr_serial *)device;
    struct mr_driver *driver = _MR_DEVICE_DRIVER_GET(device);
    struct mr_serial_driver_ops *ops = _MR_DRIVER_OPS_GET(driver);
    ...
    if ((mr_fifo_size_get(&serial->wfifo) != 0) &&
        ((ops->send_int_configure != NULL) || (ops->send_dma != NULL)))
#else
    if ((mr_fifo_size_get(&serial->wfifo) != 0) && (ops->send_int_configure != NULL))
#endif /* MR_USE_SERIAL_DMA */
    {
...
static int serial_ioctl(struct mr_device *device, int pos, int cmd, void *args)
{
    ...
    switch (cmd)
    {
        ...
        case MR_CMD_SERIAL_RD_FIFO_SIZE:
        {
            ...
            int ret = _serial_fifo_allocate(&serial->rfifo, &serial->rfifo_size);
            if (ret < 0)
            {
                return ret;
            }
        ...
        case MR_CMD_SERIAL_WR_FIFO_SIZE:
        {
            ...
            int ret = _serial_fifo_allocate(&serial->wfifo, &serial->wfifo_size);
            if (ret < 0)
            {
                return ret;
            }
...
static int serial_isr(struct mr_device *device, uint32_t event, void *args)
{
    ...
    switch (event)
    {
        ...
        {
            size_t dma_sizeof = sizeof(serial->rdma) / 2;
            size_t count = dma_sizeof;
            uint8_t *rdma, *ndma;
            ...
            return ops->receive_dma(driver, true, ndma, dma_sizeof);
        }
        case MR_EVENT_SERIAL_WR_DATA_DMA:
        {
            ...
            size_t count = mr_fifo_peek(&serial->wfifo, serial->wdma, sizeof(serial->wdma));
            if (count == 0)
            {
...
#define MR_CFG_SERIAL_WR_FIFO_SIZE      (0)
    ...
    return mr_device_register((struct mr_device *)serial, path,
                              MR_DEVICE_TYPE_SERIAL | MR_DEVICE_TYPE_FDX, &ops, driver);
}
