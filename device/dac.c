...
MR_INLINE int dac_channel_set_configure(struct mr_dac *dac,
                                        int channel,
...
MR_INLINE int dac_channel_get_configure(struct mr_dac *dac,
                                        int channel,
                                        struct mr_dac_config *config)
{
    if ((channel < 0) || (channel >= (sizeof(dac->channels) * 8))) {
...
static ssize_t mr_dac_write(struct mr_dev *dev, const void *buf, size_t count)
{
    ...
    if ((dev->position < 0) || (MR_BIT_IS_SET(dac->channels, (1 << dev->position)) == MR_DISABLE)) {
        return MR_EINVAL;
    }
...
{
    static struct mr_dev_ops ops = {mr_dac_open,
                                    mr_dac_close,
                                    MR_NULL,
                                    mr_dac_write,
                                    mr_dac_ioctl,
